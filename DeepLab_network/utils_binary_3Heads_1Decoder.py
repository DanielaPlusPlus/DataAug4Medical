...
class _SimpleSegmentationModel(nn.Module):
    ...
    def forward(self, x, train=False, superpixel_map=None):  #(16,3,224,224)
        input_shape = x.shape[-2:]  #torch.size([224,224])
        features = self.backbone(x)  #OrderedDict "low_level"(16,256,56,56); "out"(16,2048,14,14)

        if train:
            seg_out, reconstruction_out, local_cls_out = self.classifier(features, train, superpixel_map) ##(2,1,56,56), (2,3,56,56), (n,64)
            # print(seg_out.shape, reconstruction_out.shape, local_cls_out.shape)
            seg_out = F.interpolate(seg_out, size=input_shape, mode='bilinear', align_corners=False) #(2,150,224,224)
            reconstruction_out = F.interpolate(reconstruction_out, size=input_shape, mode='bilinear', align_corners=False)  # (2,3,224,224)
            seg_out = F.sigmoid(seg_out.squeeze(1))  # binary, add sigmoid
            return seg_out, reconstruction_out, local_cls_out
        else:
            seg_out = self.classifier(features, train)  # (2,1,56,56)
            seg_out = F.interpolate(seg_out, size=input_shape, mode='bilinear', align_corners=False) #(2,150,224,224)
            seg_out = F.sigmoid(seg_out.squeeze(1))  # binary, add sigmoid
            return seg_out
...
class IntermediateLayerGetter(nn.ModuleDict):
    ...
    def forward(self, x): #x是input image, (3,224,224)
        out = OrderedDict() #{feature['low_level'], feature['out']}
