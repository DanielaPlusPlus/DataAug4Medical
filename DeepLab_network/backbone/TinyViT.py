...
__all__ = ['tiny_vit_5m_224', 'tiny_vit_11m_224', 'tiny_vit_21m_224', 'tiny_vit_21m_384', 'tiny_vit_21m_512']
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x):
        # x: (N, C, H, W)
        fea = self.patch_embed(x) #将image转化为patch embeddings
        # print(fea.shape) torch.Size([96, 64, 56, 56])

        x = self.layers[0](fea) #torch.Size([96, 784, 128])
        start_i = 1

        for i in range(start_i, len(self.layers)):
            layer = self.layers[i]
            x = layer(x)
        # print(x.shape) #torch.Size([96, 49, 448])

        x = x.mean(1)
        # print(x.shape) #torch.Size([96, 448])
