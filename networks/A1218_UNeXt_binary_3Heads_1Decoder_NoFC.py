...
class UNext_3Heads_1Decoder(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False ,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        self.seg_head = nn.Conv2d(16, 1, kernel_size=1, stride=1, padding=0)
        # self.cls_head = nn.Linear(16, 64)  #local projection head for cosface classification head
        self.rec_head = nn.Conv2d(16, 3, kernel_size=1, stride=1, padding=0)



    def forward(self, x, train=False, superpixel_map=None):
        # print("input img size: ", x.shape)
        ...
        # print("decoded feature size: ", out.shape)

        # # out = self.final(out)
        # out = F.sigmoid(self.final(out).squeeze(1))

        seg_out = torch.sigmoid(self.seg_head(out).squeeze(1))
        if train==False:
            return seg_out
        else:
            recontrust_out = self.rec_head(out)
            SPAttention_fea_batch = self.SP(out, superpixel_map)  # list(32*[n, 256])
            SurPLocalCls_batch = []
...
# EOF
model = UNext_3Heads_1Decoder(num_classes=1, input_channels=3, deep_supervision=True).cuda()
