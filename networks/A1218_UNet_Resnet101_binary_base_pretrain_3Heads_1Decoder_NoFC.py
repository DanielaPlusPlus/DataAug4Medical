...
resnet = torchvision.models.resnet.resnet101(pretrained=False)
...
class UNet_R101_3Heads_1Decoder(nn.Module):
    DEPTH = 6
    ...
    def forward(self, x, train=False, superpixel_map=None):
        ...
        for i, block in enumerate(self.down_blocks, 2):
            x = block(x)
            if i == (UNet_R101_3Heads_1Decoder.DEPTH - 1):
                continue
        ...
        for i, block in enumerate(self.up_blocks, 1):
            key = f"layer_{UNet_R101_3Heads_1Decoder.DEPTH - 1 - i}"
...
model = UNet_R101_3Heads_1Decoder().cuda()
