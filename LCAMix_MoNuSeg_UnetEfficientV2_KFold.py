...
# from networks import A1215_UNetResNet50_binary_base as SegNet
from networks.A1219_EfficientNet_UNet_binary_3Heads_1Decoder_NoFC import *
import utils
import os
import random
import argparse
...
def make_print_to_file(path='./'):
    ...
    sys.stdout = Logger(
        fileName + 'A0103_MoNuSeg_UNetEfficientV2_ours_NoFC.log', path=path)
...
def get_argparser():
    ...
    parser.add_argument("--model", type=str, default='UNetEfficientV2_3Heads_pretrain',help='model name')
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(filelists)):
        ...
        model_name = './checkpoints/A1225_best_%s_%s_3Heads_1Decoder_UNPRETRAIN_%sFold.pth' %  (opts.model, opts.dataset, fold)
        if len(opts.gpu_ids) > 1:
            if opts.RESUME:
                model = get_efficientunet_3heads_1decoder_b2(out_channels=3, concat_input=True, pretrained=True)
        ...
        results_file_name = results_out_dir + tm + 'A0103_MoNuSeg_UNetEfficientV2_ours_NoFC_%sFold_results.txt' %  (fold)
